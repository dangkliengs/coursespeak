...
async function backupCurrentData() {
  ...
  try {
    ...
    const backupFile = path.join(BACKUP_DIR, `deals_local_before_full_sync_${timestamp}.json`);
...
async function saveSync(deals, timestamp) {
  ...
  const backupFile = path.join(BACKUP_DIR, `deals_full_sync_${timestamp}.json`);
  await fs.promises.writeFile(backupFile, JSON.stringify(deals, null, 2), 'utf-8');
  console.log(`📦 Full sync backup created: ${backupFile}`);
...
async function main() {
  console.log('🔄 Starting FULL sync from live coursespeak.com...\n');
  ...
  try {
    ...
    console.log('💡 Your local data now matches the live site.');
    console.log('🚀 Run "npm run dev" to test with the synced data.');
...
if (args.includes('--help') || args.includes('-h')) {
  console.log(`
🔄 Full Live Sync Script

Usage: node scripts/sync-full-from-live.js [options]
...
This ensures your local data is IDENTICAL to the live site.
`);
  process.exit(0);
}
